        ...
        for(int X = XMin; X <= XMax; ++X)
        {
               ...
               (Edge3 < 0))
            {
                ...
                if(NormalMap)
                {
                    ...
                    v3 LightColor = {0, 0, 0}; // SamepleEnvironmentMap(ScreenSpaceUV, Normal.xyz, Normal.w, Middle);
                    if(FarMap)
                    {
                        v3 FarMapColor = SamepleEnvironmentMap(ScreenSpaceUV, Normal.xyz, Normal.w, FarMap);
