...
inline v3 SamepleEnvironmentMap(v2 ScreenSpaceUV, v3 SampleDirection, real32 Roughness, environment_map *Map, real32 DistanceFromMapInZ)
{
     /* NOTE:

        ScreenSpaceUV tells us where the ray is being cast _from_ in normalized screnn coordinates.

        SampleDirection tells us what direction the cast is going - it does not have to be normalized.

        Roughness says which LODs of Map we sample from.

        DistanceFromMapInZ says how far the map is from the sample point in Z, given in meters.
    */

    // NOTE: Pick whihc LOD to sample from
    uint32 LODIndex = (uint32)(Roughness * (real32)(ArrayCount(Map->LOD) - 1) + 0.5f);
    Assert(LODIndex < ArrayCount(Map->LOD));

    loaded_bitmap *LOD = &Map->LOD[LODIndex];

    // NOTE: Compute the distance to the map and the scaling factor in meters-to-UVs
    real32 UVsPerMeter = 0.1f; // TOOD: Patameterize this, and should be different for X and Y based on map!
    real32 C = (UVsPerMeter* DistanceFromMapInZ) / SampleDirection.y;
    v2 Offset = C * V2(SampleDirection.x, SampleDirection.z);

    // NOTE: Find the interection point
...
internal void DrawRectangleQuickly(loaded_bitmap *Buffer, v2 Origin, v2 XAxis, v2 YAxis, v4 Color, loaded_bitmap *Texture, real32 PixelsToMeters, rectangle2i ClipRect, bool32 Even)
{
    ...
    if(HasArea(FillRect))
    {
        ...
        {
            _mm_slli_si128(EndClipMask, 0 * 4),
            _mm_slli_si128(EndClipMask, 3 * 4),
            _mm_slli_si128(EndClipMask, 2 * 4),
            _mm_slli_si128(EndClipMask, 1 * 4),
        };
...
internal void DrawRectangleSlowly(loaded_bitmap *Buffer, v2 Origin, v2 XAxis, v2 YAxis, v4 Color, loaded_bitmap *Texture,
                                  loaded_bitmap *NormalMap, environment_map *Top, environment_map *Middle, environment_map *Bottom, real32 PixelsToMeters)
{
        ...
        for(int X = XMin; X <= XMax; ++X)
        {
               ...
               (Edge3 < 0))
            {
                ...
                if(NormalMap)
                {
                    ...
                    // This is just the simplified version of the reflection of -e + 2e^T N N
                    v3 BounceDirection = 2.0f * Normal.z * Normal.xyz;
                    BounceDirection.z -= 1.0;
                    ...
                    if(FarMap)
                    {
                        ...
                        v3 FarMapColor = SamepleEnvironmentMap(ScreenSpaceUV, BounceDirection, Normal.w, FarMap, DistanceFromMapInZ);
    ...
    for(uint32 BaseAddress = 0; BaseAddress < RenderGroup->PushBufferSize;)
    {
        ...
        switch (Header->Type)
        {
            case RenderGroupEntryType_render_entry_clear:
            {
                ...
                DrawRectangle(OutputTarget, V2(0.0f, 0.0f), V2((real32)OutputTarget->Width, (real32)OutputTarget->Height), Entry->Color, ClipRect, Even);
...
internal void TiledRenderGroupToOutput(platform_work_queue *RenderQueue, render_group *RenderGroup, loaded_bitmap *OutputTarget)
{
    /*
        ...
        - How big shuold the tiles be for performance?
...
internal render_group *AllocateRenderGroup(memory_arena *Arena, uint32 MaxPushBufferSize, uint32 ResolutionPixelsX, uint32 ResolutionPixelsY)
{
...
inline void CoordinateSystem(render_group *Group, v2 Origin, v2 XAxis, v2 YAxis, v4 Color, loaded_bitmap *Texture, loaded_bitmap *NormalMap, environment_map *Top, environment_map *Middle, environment_map *Bottom)
{
#if 0
