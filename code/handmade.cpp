...
#if 0
    // TODO: Re-enable this?  But make sure we don't touch ones in flight??
        ...
        if(Entity->Updatable)
        {
            ...
            switch(Entity->Type)
            {
                case EntityType_Hero:
                {
                    ...
                    // NOTE: Particle systems test
                    ZeroStruct(GameState->ParticleCels);

                    r32 GridScale = 0.25f;
                    r32 InvGridScale = 1.0f / GridScale;
                    v3 GridOrigin = {(real32)(-0.5 * GridScale * PARTICLE_CEL_DIM), 0.0f, 0.0f};
                    ...
                    for(u32 ParticleIndex = 0; ParticleIndex < ArrayCount(GameState->Particles); ++ParticleIndex)
                    {
                        ...
                        Particle->P += (0.5f * Square(Input->dtForFrame) * Input->dtForFrame * Particle->dP) +
                        ...
                        {
                            r32 CoefficientOfRestitution = 0.3;
