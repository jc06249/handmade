...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    if(!Memory->IsInitialised)
    {
        ...
        GameState->HeroHead = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_front_head.bmp");
        GameState->HeroCape = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_front_cape.bmp");
        GameState->HeroTorso = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_front_torso.bmp");
    ...
    for(int RelRow = -10; RelRow < 10; ++RelRow)
    {
        for(int RelColumn = -20; RelColumn < 20; ++RelColumn)
        {
            ...
            if(TileID > 1)
            {
                ...
                real32 CenX = SreenCenterX - (MetersToPixels * GameState->PlayerP.OffsetX) + ((real32)RelColumn) * TileSideInPixels;
                real32 CenY = SreenCenterY + (MetersToPixels * GameState->PlayerP.OffsetY) - ((real32)RelRow) * TileSideInPixels;
    ...
    real32 PlayerG = 1.0f;
    real32 PlayerB = 0.0f;
    real32 PlayerLeft = SreenCenterX  - (0.5f * MetersToPixels * PlayerWidth);
    real32 PlayerTop = SreenCenterY - (MetersToPixels * PlayerHeight);
