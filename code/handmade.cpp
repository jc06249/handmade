...
internal void DrawBitmap(game_offscreen_buffer *Buffer, loaded_bitmap *Bitmap, real32 RealX, real32 RealY, int32 AlignX = 0, int32 AlignY = 0, real32 CAlpha = 1.0f)
{
    ...
    // TODO: SourceRow needs to be changed baed on clipping.
    ...
    for(int32 Y = MinY; Y < MaxY; ++Y)
    {
        ...
        for(int32 X = MinX; X < MaxX; ++X)
        {
            ...
            // NOTE: Someday, we need to talk about premultiplied alpha!
...
internal loaded_bitmap DEBUGLoadBMP(thread_context *Thread, debug_platform_read_entire_file *ReadEntireFile, char *FileName)
{
    ...
    if(ReadResult.ContentsSize != 0)
    {
        ...
        Assert(Header->Compresssion ==3);

        // NOTE: If you are using this generically for some reason,
        // please remember that BMP files CAN GO IN EITHER DIRECTION and
        // the height will be negative for top-down.
        // (Also, there can be compresssion, etc., etc... DON'T think this
        // is complete BMP loading code because it isn't!!)

        // NOTE: Byte order ni memory is determines by the Header itself,
        // so we have to read out the masks and convert the puxels ourselves.
...
internal void ChangeEntityResidence(game_state *GameState, uint32 EntityIndex, entity_residence Residence)
{
    ...
    if(Residence == EntityResidence_High)
    {
        if(GameState->EntityResidence[EntityIndex] != EntityResidence_High)
        {
            ...
            // NOTE: Make the entity into camera space
...
internal uint32 AddEntity(game_state *GameState, entity_type Type)
{
    ...
    GameState->EntityResidence[EntityIndex] = EntityResidence_Dorment;
...
internal uint32 AddWall(game_state *GameState, uint32 AbsTileX, uint32 AbsTileY, uint32 AbsTileZ)
{
    ...
    entity Entity = GetEntity(GameState, EntityResidence_Dorment, EntityIndex);
...
internal uint32 AddPlayer(game_state *GameState)
{
    ...
    if(GetEntity(GameState, EntityResidence_Dorment, GameState->CameraFollowingEntityIndex).Residence == EntityResidence_Nonexistent)
    {
...
internal void MovePlayer(game_state *GameState, entity Entity, real32 dt, v2 ddP)
{
    ...
    for(uint32 Iteration = 0; (Iteration < 4); ++Iteration)
    {
        ...
        if(HitEntityIndex)
        {
            ...
            entity HitEntity = GetEntity(GameState, EntityResidence_Dorment, HitEntityIndex);
    ...
    // TODO: Change this to using the acceleration vector
...
internal void SetCamera(game_state *GameState, tile_map_position NewCameraP)
{
    ...
    rectangle2 CameraBounds = RectCenterHalfDim(V2(0, 0), TileMap->TileSideInMeters * V2((real32)TileSpanX, (real32)TileSpanY));
    ...
    for(uint32 EntityIndex = 1; EntityIndex < ArrayCount(GameState->HighEntities); ++EntityIndex)
    {
        if(GameState->EntityResidence[EntityIndex] == EntityResidence_High)
        {
            ...
            {
                ChangeEntityResidence(GameState, EntityIndex, EntityResidence_Dorment);
            }
        }
    }
    ...
    {
        if(GameState->EntityResidence[EntityIndex] == EntityResidence_Dorment)
        {
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    if(!Memory->IsInitialised)
    {
        ...
        GameState->Shadow = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_shadow.bmp");

        hero_bitmaps *Bitmap;

        Bitmap = GameState->HeroBitmaps;
        Bitmap->Head = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_right_head.bmp");
        Bitmap->Cape = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_right_cape.bmp");
        Bitmap->Torso = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_right_torso.bmp");
        Bitmap->AlignX = 72;
        Bitmap->AlignY = 182;
        ++Bitmap;

        Bitmap->Head = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_back_head.bmp");
        Bitmap->Cape = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_back_cape.bmp");
        Bitmap->Torso = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_back_torso.bmp");
        Bitmap->AlignX = 72;
        Bitmap->AlignY = 182;
        ++Bitmap;

        Bitmap->Head = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_left_head.bmp");
        Bitmap->Cape = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_left_cape.bmp");
        Bitmap->Torso = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_left_torso.bmp");
        Bitmap->AlignX = 72;
        Bitmap->AlignY = 182;
        ++Bitmap;

        Bitmap->Head = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_front_head.bmp");
        Bitmap->Cape = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_front_cape.bmp");
        Bitmap->Torso = DEBUGLoadBMP(Thread, Memory->DEBUGPlatformReadEntireFile, "../handmade/data/test/test_hero_front_torso.bmp");
        Bitmap->AlignX = 72;
        Bitmap->AlignY = 182;
        ++Bitmap;
    ...
    for(int RelRow = -10; RelRow < 10; ++RelRow)
    {
        for(int RelColumn = -20; RelColumn < 20; ++RelColumn)
        {
            ...
            if(TileID > 1)
            {
                ...
                v2 Min = Cen - 1.0f * TileSide;
                v2 Max = Cen + 1.0f * TileSide;
    ...
    for(uint32 EntityIndex = 1; EntityIndex < GameState->EntityCount; ++EntityIndex)
    {
        if(GameState->EntityResidence[EntityIndex] == EntityResidence_High)
        {
            ...
            v2 PlayerLeftTop = {PlayerGroundPointX - (0.5f * MetersToPixels * DormantEntity->Width), PlayerGroundPointY - (0.5f * MetersToPixels * DormantEntity->Height)};
