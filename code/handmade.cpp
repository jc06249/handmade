...
internal void MovePlayer(game_state *GameState, entity *Entity, real32 dt, v2 ddP)
{
    ...
    v2 PlayerDelta = 0.5f * ddP * Square(dt) + // 0.5 * velocity * dt^2
                        Entity->dP * dt;
    NewPlayerP.Offset += PlayerDelta;
    Entity->dP = ddP * dt + Entity->dP;
    NewPlayerP = RecanonicalizePosition(TileMap, NewPlayerP);
    // TODO: Delta function that auto-recanonicalises
    ...
    for(uint32 AbsTileY = MinTileY; AbsTileY != OnePastMaxTileY; ++AbsTileY)
    {
        for(uint32 AbsTileX = MinTileX; AbsTileX != OnePastMaxTileX; ++AbsTileX)
        {
            ...
            if(IsTileValueEmpty(TileValue))
            {
                ...
                // TODO: Test all four walls and the the minimum Z.
                tResult = (WallX - RelNewPlayerP.X) / PlayerDelta.X;
                TestWall(MinCorner.X, MinCorner.Y, MaxCorner.Y, MaxCorner.X);
            }
        }
    }
#endif
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    for(int RelRow = -10; RelRow < 10; ++RelRow)
    {
        for(int RelColumn = -20; RelColumn < 20; ++RelColumn)
        {
            ...
            if(TileID > 1)
            {
                ...
                v2 Cen = {SreenCenterX - (MetersToPixels * GameState->CameraP.Offset.X) + ((real32)RelColumn) * TileSideInPixels,
                          SreenCenterY + (MetersToPixels * GameState->CameraP.Offset.Y) - ((real32)RelRow) * TileSideInPixels};
                v2 Min = Cen - TileSide;
                v2 Max = Cen + TileSide;
    ...
    for(uint32 EntityIndex = 0; EntityIndex < GameState->EntityCount; ++EntityIndex, ++Entity)
    {
        if(Entity->Exists)
        {
            ...
            real32 PlayerG = 1.0f;
            real32 PlayerB = 0.0f;
            real32 PlayerGroundPointX = SreenCenterX + MetersToPixels * Diff.dXY.X;
            real32 PlayerGroundPointY = SreenCenterY - MetersToPixels * Diff.dXY.Y;
