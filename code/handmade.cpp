    ...
    for(int RelRow = -10; RelRow < 10; ++RelRow)
    {
        for(int RelColumn = -20; RelColumn < 20; ++RelColumn)
        {
            ...
            if(TileID > 0)
            {
                ...
                real32 CenX = SreenCenterX - (MetersToPixels * GameState->PlayerP.TileRelX) + ((real32)RelColumn) * TileSideInPixels;
                real32 CenY = SreenCenterY + (MetersToPixels * GameState->PlayerP.TileRelY) - ((real32)RelRow) * TileSideInPixels;
