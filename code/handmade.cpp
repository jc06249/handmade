...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    real32 SreenCenterX = 0.5f * (real32)Buffer->Width;
    real32 SreenCenterY = 0.5f * (real32)Buffer->Height;
    ...
    for(int RelRow = -10; RelRow < 10; ++RelRow)
    {
        for(int RelColumn = -20; RelColumn < 20; ++RelColumn)
        {
            ...
            if(TileID > 1)
            {
                ...
                v2 Cen = {SreenCenterX - (MetersToPixels * GameState->CameraP.Offset_.X) + ((real32)RelColumn) * TileSideInPixels,
                          SreenCenterY + (MetersToPixels * GameState->CameraP.Offset_.Y) - ((real32)RelRow) * TileSideInPixels};
    ...
    for(uint32 EntityIndex = 0; EntityIndex < GameState->EntityCount; ++EntityIndex)
    {
        if(GameState->EntityResidence[EntityIndex] == EntityResidence_High)
        {
            ...
            real32 PlayerG = 1.0f;
            real32 PlayerB = 0.0f;
            real32 PlayerGroundPointX = SreenCenterX + MetersToPixels * HighEntity->P.X;
            real32 PlayerGroundPointY = SreenCenterY - MetersToPixels * HighEntity->P.Y;
