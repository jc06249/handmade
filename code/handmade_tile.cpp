...
inline tile_chunk * GetTileChunk(tile_map *TileMap, int32 TileChunkX, int32 TileChunkY, int32 TileChunkZ, memory_arena *Arena = 0)
{
    ...
    do
    {
        ...
        if(Arena && (Chunk->TileChunkX != TILE_CHUNK_UNITIALISED) && (!Chunk->NextInHash))
        {
            Chunk->NextInHash = PushStruct(Arena, tile_chunk);
            Chunk = Chunk->NextInHash;
            Chunk->TileChunkX = TILE_CHUNK_UNITIALISED;
        }

        if(Arena && (Chunk->TileChunkX == TILE_CHUNK_UNITIALISED))
        {
            ...
            // TODO: Do we want to always initialise?
...
inline void SetTileValue(tile_map *TileMap, tile_chunk *TileChunk, int32 TestTileX, int32 TestTileY, int32 TileValue)
{
...
internal void InitialiseTileMap(tile_map *TileMap, real32 TileSideInMeters)
{
    ...
    {
        TileMap->TileChunkHash[TileChunkIndex].TileChunkX = TILE_CHUNK_UNITIALISED;
    }
}
